...
char fchars[200]; // buffer for printing floats
char fchars2[200]; // buffer for printing float
...
uint32_t* registers_base = (uint32_t*) 0x00042000UL;
...
//					UART_send(&g_uart, outBuffer ,bufcount );
				...
				if (commandID == ADCRWCMDID){
					...
					//for (int i=0;i<num_bits;i++){
						*(register_base_addr + 0x30) = ((mapped_channel_mask1 & 0xFF)>>0);
						*(register_base_addr + 0x31) = ((mapped_channel_mask1 & 0xFF00)>>8);
						*(register_base_addr + 0x32) = ((mapped_channel_mask1 & 0xFF0000)>>16);
						*(register_base_addr + 0x33) = ((mapped_channel_mask1 & 0xFF000000)>>24);
						*(register_base_addr + 0x34) = ((mapped_channel_mask2 & 0xFF)>>0);
						*(register_base_addr + 0x35) = ((mapped_channel_mask2 & 0xFF00)>>8);
						*(register_base_addr + 0x30) = 0x0;
						*(register_base_addr + 0x31) = 0x0;
						*(register_base_addr + 0x32) = 0x0;
						*(register_base_addr + 0x33) = 0x0;
						*(register_base_addr + 0x34) = 0x0;
						*(register_base_addr + 0x35) = 0x0;
						delayUs(100);
					//}
					...
					UART_polled_tx_string( &g_uart, outBuffer );
					*/
					volatile uint32_t *empty_p = (register_base_addr + 0x43);
					volatile uint32_t *full_p = (register_base_addr + 0x42);
					volatile uint32_t *data_p = (register_base_addr + 0x41);
