...
typedef struct{
    ...
    uint32_t _tx_buffer;  //buffer for outgoing data
    uint32_t _rx_buffer;  //buffer for receiving data
} SPI_daisy;
...
uint32_t SPI_daisy_read(SPI_daisy* self, uint8_t nbyte); //maximal 4 byte
void SPI_daisy_write(SPI_daisy* self, uint8_t nbyte, uint32_t data); //maximal 4 byte
