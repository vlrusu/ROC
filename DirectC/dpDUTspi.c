...
void do_SPI_SCAN_in(unsigned char command, unsigned int data_bits, unsigned char *input_buffer)
{
    ...
    SPI_set_slave_select( DUT_SPI_INSTANCE, DUT_SPI_SLAVE );
    SPI_transfer_block( DUT_SPI_INSTANCE, tmp_buf, (data_bits + 8) / 8, DPNULL, 0u);
    SPI_clear_slave_select( DUT_SPI_INSTANCE, DUT_SPI_SLAVE );
...
{
    SPI_set_slave_select( DUT_SPI_INSTANCE, DUT_SPI_SLAVE );
    SPI_transfer_block( DUT_SPI_INSTANCE, command, command_bits/8u, output_buffer, data_bits/8u );
    SPI_clear_slave_select( DUT_SPI_INSTANCE, DUT_SPI_SLAVE );
