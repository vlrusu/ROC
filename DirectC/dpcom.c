...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
    ...
    {
        DPUCHAR addressOffset[4]={0x0, 0x0, 0x0, 0x0};
        DPUCHAR numBytes[4];
            DPULONG rx_size = 0;
 ...
 //           while(MSS_UART_tx_complete(gp_my_uart) == 0);
            ...
            addressOffset[0] = image_requested_address & 0xff;
            addressOffset[1] = (image_requested_address >> 8) & 0xff;
            addressOffset[2] = (image_requested_address >> 16) & 0xff;
            addressOffset[3] = (image_requested_address >> 24) & 0xff;
            
            UART_send (&g_uart, addressOffset, 4);

 //           while(MSS_UART_tx_complete(gp_my_uart) == 0);
            
            numBytes[0] = return_bytes & 0xff;
            numBytes[1] = (return_bytes >> 8) & 0xff;
            numBytes[2] = (return_bytes >> 16) & 0xff;
            numBytes[3] = (return_bytes >> 24) & 0xff;
...
//            while(MSS_UART_tx_complete(gp_my_uart) == 0);
            ...
            {
                rx_size += UART_get_rx(&g_uart, &page_global_buffer[rx_size],return_bytes);
            }
